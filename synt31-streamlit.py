...
def calculate_mse_for_optimization_penalized(ep_vector, nH, nL, nSub, l_vec_optim, active_targets, min_thickness_phys_nm):
    ...
    if np.any(below_min_mask):
        penalty = 1e6 + np.sum((min_thickness_phys_nm - ep_vector[below_min_mask])**2) * 1e8
    try:
        nH_complex = nH + 0j if isinstance(nH, (int, float)) else nH
        nL_complex = nL + 0j if isinstance(nL, (int, float)) else nL
        nSub_complex = nSub + 0j if isinstance(nSub, (int, float)) else nSub
...
def tracer_graphiques(res, ep_actual, nH_r, nH_i, nL_r, nL_i, nSub, active_targets_for_plot, mse,
    ...
    if res and 'l' in res and 'Ts' in res and res['l'] is not None and res['Ts'] is not None:
        ...
        if target_lines_drawn or ax_spec.get_legend_handles_labels()[1]:
    ...
    if num_layers > 0:
        for i in range(num_layers):
            layer_start_depth = ep_cum[i-1] if i > 0 else 0
    ...
    if num_layers > 0:
        ...
        max_ep = max(ep_actual) if ep_actual.size > 0 else 1.0
...
with col1:
    with st.expander("Materials and Substrate", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            st.number_input("Material H (n real)", min_value=0.0, value=2.35, step=0.01, format="%.3f", key='nH_r')
            st.number_input("Material L (n real)", min_value=0.0, value=1.46, step=0.01, format="%.3f", key='nL_r')
            st.number_input("Substrate (n real)", min_value=0.0, value=1.52, step=0.01, format="%.3f", key='nSub')
        with c2:
...
with col2:
    with st.expander("Calculation & Optimization Parameters", expanded=True):
        st.radio(
            ...
            index=list(OPTIMIZATION_MODES.keys()).index(st.session_state.optim_mode),
            key='optim_mode_radio',
            horizontal=True
        )
        st.session_state.optim_mode = st.session_state.optim_mode_radio # Update state from radio
        ...
        with c1:
            st.number_input("λ Start (nm)", min_value=0.1, value=400.0, step=1.0, format="%.1f", key='l_range_deb_input')
        with c2:
            st.number_input("λ End (nm)", min_value=0.1, value=700.0, step=1.0, format="%.1f", key='l_range_fin_input')
        with c3:
            st.number_input("λ Step (nm, Optim Grid)", min_value=0.01, value=10.0, step=0.1, format="%.2f", key='l_step_input')
            st.caption("Plot uses λ Step / 10")

        c1, c2, c3 = st.columns(3)
        with c1:
            st.text_input("N Samples (Sobol)", value=selected_params['n_samples'], key='n_samples_input')
        with c2:
            st.text_input("P Starts (L-BFGS-B)", value=selected_params['p_best'], key='p_best_input')
        with c3:
            st.text_input("Optim Passes", value=selected_params['n_passes'], key='n_passes_input')
...
with action_cols[2]:
    remove_layer_enabled = (st.session_state.current_optimized_ep is not None and
                            len(st.session_state.current_optimized_ep) > 1 and # Allow removal down to 1 layer
...
with action_cols[3]:
    set_nominal_enabled = (st.session_state.current_optimized_ep is not None and
                           len(st.session_state.current_optimized_ep) > 0 and # Allow setting if any layers exist
                           st.session_state.optimization_ran_since_nominal_change)
...
def _prepare_calculation_data_st(inputs, ep_vector_to_use=None):
    ...
    if ep_actual_orig.size == 0:
        ...
        if not np.all(np.isfinite(ep_actual_orig)):
            raise ValueError(f"Thickness vector contains non-finite values: {ep_actual_orig}")
...
def run_optimization_process(inputs, active_targets, ep_nominal_for_cost,
                             p_best_this_pass, ep_reference_for_sobol=None,
                             current_scaling=None, run_number=1, total_passes=1,
                             progress_bar_hook=None, status_placeholder=None,
                             current_best_mse=np.inf, current_best_layers=0):
    ...
    if ep_reference_for_sobol is None:
    ...
    valid_initial_results_p1 = _run_sobol_evaluation(
        ...
        progress_hook=None # Pass hook if needed
    )
    ...
    if not selected_starts:
        ...
        elif ep_nominal_for_cost is not None:
             overall_best_ep = ep_nominal_for_cost.copy()
             try: overall_best_cost = cost_function_partial_map(overall_best_ep)
             except Exception: overall_best_cost = np.inf
        else: overall_best_ep = None; overall_best_cost = np.inf
        ...
        local_results_raw = _run_parallel_local_search(selected_starts,
                                                        ...
                                                        args_for_cost_tuple, lbfgsb_bounds,
                                                        MIN_THICKNESS_PHYS_NM, progress_hook=None)
    ...
    try:
        ...
        for pass_num in range(1, n_passes + 1):
            ...
            if pass_num == 1:
                ...
                scale_factor = 1.8**(pass_num - 2)
                current_scaling = max(min_scaling_nm, initial_scaling_nm / scale_factor)
                ep_ref_sobol = ep_ref_for_next_pass

            ep_this_pass, cost_this_pass, result_obj_this_pass = run_optimization_process(
                inputs=inputs, active_targets=active_targets, ep_nominal_for_cost=ep_nominal_glob,
                p_best_this_pass=p_best_for_this_pass, ep_reference_for_sobol=ep_ref_sobol,
                current_scaling=current_scaling, run_number=pass_num, total_passes=n_passes,
                progress_bar_hook=progress_bar, status_placeholder=status_placeholder,
                current_best_mse=overall_best_cost_final, current_best_layers=overall_best_layers
            )
        ...
        temp_nH_ar = temp_inputs_ar['nH_r'] + 1j * temp_inputs_ar['nH_i']; temp_nL_ar = temp_inputs_ar['nL_r'] + 1j * temp_inputs_ar['nL_i']
        ...
        log_with_elapsed_time(err_msg); st.error(err_msg)
...
if remove_button:
    ...
    if current_ep is None or not optim_ran or len(current_ep) <= 1:
        ...
        try:
            ...
            nH = inputs['nH_r'] + 1j * inputs['nH_i']; nL = inputs['nL_r'] + 1j * inputs['nL_i']; nSub_c = inputs['nSub'] + 0j
            ...
            if structure_actually_changed:
                ...
                st.info("Could not find suitable layer or removal/re-optimization failed/unchanged.")
    ...
    if current_ep is None or not optim_ran or len(current_ep) == 0:
        ...
        try:
            ...
            if np.any(np.isnan(optimized_qwots)):
                ...
                st.success("Current optimized design set as new Nominal Structure (QWOT). Optimized state cleared.")
            ...
            log_message(err_msg); st.error(f"An error occurred setting nominal: {e}")
