...
def calculate_mse_for_optimization_penalized(ep_vector, nH, nL, nSub, l_vec_optim, active_targets, min_thickness_phys_nm, debug_log=False):
    if debug_log:
        log_message(f"[DEBUG COST] Input ep_vector (len {len(ep_vector)}): {np.array2string(ep_vector, precision=4, max_line_width=120)}")
    ...
    if np.any(below_min_mask):
        ...
        if debug_log:
             log_message(f"[DEBUG COST] Thickness penalty applied: {thickness_penalty:.4e}")
             log_message(f"[DEBUG COST] Violated thicknesses: {np.array2string(ep_vector[below_min_mask], precision=4)}")
    ...
    try:
        ...
        if res is None or 'Ts' not in res or np.any(~np.isfinite(res['Ts'])):
             ...
             penalty += nan_penalty
             if debug_log:
                 log_message(f"[DEBUG COST] NaN detected in Ts calculation! Applying NaN penalty: {nan_penalty:.4e}")
             if debug_log: log_message(f"[DEBUG COST] Returning early due to NaN. Total cost: {penalty:.4e}")
             return penalty
    except Exception as e_rt:
        if debug_log: log_message(f"[DEBUG COST] ERROR during calculate_RT_from_ep: {e_rt}")
        return np.inf
    ...
    if 'Ts' in res and 'l' in res:
        ...
        for target in active_targets:
            ...
            if indices.size > 0:
                ...
                squared_errors = (calculated_Ts_in_zone - interpolated_target_t)**2
                total_squared_error += np.sum(squared_errors)
                total_points_in_targets += len(valid_indices)
        if total_points_in_targets > 0: mse = total_squared_error / total_points_in_targets
    if debug_log:
        log_message(f"[DEBUG COST] Calculated MSE: {mse:.4e} (Points: {total_points_in_targets})")
        log_message(f"[DEBUG COST] Total Penalty (Thickness+NaN): {penalty:.4e}")
    ...
    if structure_changed and ep_after_merge is not None:
        ...
        try:
            reopt_result = minimize(cost_function, x0_reopt, args=reopt_args,
                                    ...
                                    options={'maxiter': 199, 'ftol': 1e-10, 'gtol': 1e-7, 'disp': False})
            ...
            if reopt_success:
                ...
                final_ep = np.maximum(ep_after_merge.copy(), min_thickness_phys)
                success_overall = False
                try:
...
def local_search_worker(start_ep, cost_function, args_for_cost, lbfgsb_bounds, min_thickness_phys_nm):
    ...
    try:
        ...
        result = minimize(
            ...
            options={'disp': False, 'maxiter': 99, 'ftol': 1.e-10, 'gtol': 1e-7},
        )
    ...
    worker_summary = (
        ...
        f"Success={success_status} (LBFGSB Success: {result.success}, Finite Cost: {np.isfinite(final_cost)}), "
...
def _run_sobol_evaluation(num_layers, n_samples, lower_bounds, upper_bounds,
    ...
    if num_invalid > 0: log_with_elapsed_time(f"  Filtering: {num_invalid} invalid initial costs discarded.")
...
def _process_optimization_results(local_results_raw, initial_best_ep, initial_best_cost):
    ...
    best_run_info = {'index': -1, 'cost': overall_best_cost}
    ...
    for i, worker_output in enumerate(local_results_raw):
        ...
        if isinstance(worker_output, dict) and 'log_lines' in worker_output:
            ...
            for line in worker_log_lines: log_with_elapsed_time(f"    (Worker Log Run {run_idx}): {line.strip()}")
        else:
        ...
        else: log_with_elapsed_time(f"  Result {run_idx}: Worker output dictionary missing required keys.")
    log_with_elapsed_time(f"--- Finished processing local search results ---")
    if processed_results_count == 0 and best_run_info['index'] == -1: log_with_elapsed_time("WARNING: No valid local search results were processed. Keeping pre-local best result.")
    ...
    elif best_run_info['index'] == -1: log_with_elapsed_time(f"No local search improved the pre-local search best result (Cost {best_run_info['cost']:.3e}).")
    if overall_best_ep is None or len(overall_best_ep) == 0:
         ...
         if initial_best_ep is None: raise RuntimeError("Result processing failed: Final thickness vector is empty, and initial was also empty.")
         else:
            log_with_elapsed_time("Falling back to initial state due to empty final vector.")
            overall_best_ep = initial_best_ep.copy(); overall_best_cost = initial_best_cost
...
if 'last_run_calculation_results' not in st.session_state: st.session_state.last_run_calculation_results = {'res': None, 'ep': None, 'mse': None, 'res_optim_grid': None}
...
with col1:
    with st.expander("Materials and Substrate", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            st.number_input("Material H (n real)", min_value=0.01, value=2.35, step=0.01, format="%.3f", key='nH_r')
            st.number_input("Material L (n real)", min_value=0.01, value=1.46, step=0.01, format="%.3f", key='nL_r')
            st.number_input("Substrate (n real)", min_value=0.01, value=1.52, step=0.01, format="%.3f", key='nSub')
        with c2:
            st.number_input("Material H (k imag)", min_value=0.0, value=0.0, step=0.001, format="%.4f", key='nH_i')
            st.number_input("Material L (k imag)", min_value=0.0, value=0.0, step=0.001, format="%.4f", key='nL_i')
            st.caption("(n = n' + ik)")
...
with col2:
    ...
    with st.expander("Spectral Target (Optimization on Transmittance T)", expanded=True):
        ...
        headers[0].markdown("**Enable**"); headers[1].markdown("**Zone**"); headers[2].markdown("**λ min (nm)**")
        headers[3].markdown("**λ max (nm)**"); headers[4].markdown("**T @ λmin**"); headers[5].markdown("**T @ λmax**")
        active_target_list = []
        for i in range(len(st.session_state.targets)):
            ...
            st.session_state.targets[i]['enabled'] = enabled; st.session_state.targets[i]['min'] = l_min_str
            st.session_state.targets[i]['max'] = l_max_str; st.session_state.targets[i]['target_min'] = t_min_str
...
if opt_button:
    ...
    min_scaling_nm = 0.1; ep_nominal_glob = None; overall_best_ep_final = None
    overall_best_cost_final = np.inf; overall_best_layers = 0; initial_nominal_cost = np.inf
    initial_nominal_layers = 0; final_successful_result_obj = None; ep_ref_for_next_pass = None
    n_passes = 0; optimization_successful = False; auto_removed_count = 0
    try:
        inputs = _validate_physical_inputs_from_state(require_optim_params=True)
        n_samples = inputs['n_samples']; initial_p_best = inputs['p_best']
        n_passes = inputs['n_passes']; initial_scaling_nm = inputs['scaling_nm']
        ...
        last_successful_ep = ep_nominal_glob.copy(); overall_best_ep_final = last_successful_ep.copy()
        ep_ref_for_next_pass = None; overall_best_layers = initial_nominal_layers
        try:
        ...
        l_vec_optim = l_vec_optim_init # Use the same one for all passes

        for pass_num in range(1, n_passes + 1):
            run_id_str = f"Pass {pass_num}/{n_passes}"
            pass_status_prefix = f"Status: {run_id_str}"; status_suffix = f"| Best MSE: {overall_best_cost_final:.3e} | Layers: {overall_best_layers}"
            st.session_state.current_status = f"{pass_status_prefix} - Starting... {status_suffix}"
            status_placeholder.info(st.session_state.current_status); progress_bar.progress( current_progress_step / progress_max_steps )
            reduction_factor = 0.8 ** (pass_num - 1)
            p_best_for_this_pass = max(2, int(np.round(initial_p_best * reduction_factor)))
            current_scaling = None; ep_ref_sobol = None
            if pass_num == 1: current_scaling = None; ep_ref_sobol = None
            ...
            args_for_cost_tuple_sobol = (inputs['nH_r'] + 1j*inputs['nH_i'], inputs['nL_r'] + 1j*inputs['nL_i'], inputs['nSub'] + 0j, l_vec_optim, active_targets, MIN_THICKNESS_PHYS_NM, False) # No debug log for Sobol
            ...
            valid_initial_results_p1 = _run_sobol_evaluation(len(ep_ref_sobol) if ep_ref_sobol is not None else initial_nominal_layers, n_samples, lower_bounds, upper_bounds, cost_function_partial_map, MIN_THICKNESS_PHYS_NM, phase_name=f"{run_id_str} Phase 1")
            top_p_results_combined = []
            if run_number == 1:
                status_placeholder.info(f"{pass_status_prefix} - Phase 1bis (Refining Starts) {status_suffix}")
                ...
                if num_to_select_p1 == 0: log_with_elapsed_time("WARNING: Phase 1 yielded no valid starting points. Skipping refinement."); top_p_results_combined = []
                    ...
                    n_samples_per_point = upper_power_of_2(n_samples_per_point_raw); log_with_elapsed_time(f"  Generating {n_samples_per_point} samples around each of the top {num_to_select_p1} points (scaling +/- {phase1bis_scaling} nm).")
                    phase1bis_candidates = []
                    current_num_layers = len(top_p_starts_p1[0])
                    for i, ep_start in enumerate(top_p_starts_p1):
                        lower_bounds_1bis = np.maximum(MIN_THICKNESS_PHYS_NM, ep_start - phase1bis_scaling); upper_bounds_1bis = ep_start + phase1bis_scaling
                        upper_bounds_1bis = np.maximum(upper_bounds_1bis, lower_bounds_1bis + 0.1)
                        ...
                        new_candidates = qmc.scale(points_unit_1bis, lower_bounds_1bis, upper_bounds_1bis); new_candidates = [np.maximum(MIN_THICKNESS_PHYS_NM, cand) for cand in new_candidates]
                    ...
                    try:
                        if phase1bis_candidates:
                            ...
                            eval_pool_time_1bis = time.time() - eval_start_pool_1bis; log_with_elapsed_time(f"  Parallel evaluation (Phase 1bis) finished in {eval_pool_time_1bis:.2f}s.")
                    ...
                    except Exception as e_pool_1bis: log_with_elapsed_time(f"  ERROR during parallel evaluation (Phase 1bis): {e_pool_1bis}"); results_1bis_raw_pairs = []
                    ...
                    combined_results = top_p_results_p1 + valid_results_1bis; log_with_elapsed_time(f"  Combined Phase 1 ({len(top_p_results_p1)}) and Phase 1bis ({len(valid_results_1bis)}) results: {len(combined_results)} total.")
                    if not combined_results: log_with_elapsed_time("WARNING: No valid results found after combining Phase 1 and Phase 1bis."); top_p_results_combined = []
                    else: combined_results.sort(key=lambda x: x[0]); num_to_select_final = min(p_best_for_this_pass, len(combined_results)); log_with_elapsed_time(f"  Selecting final top {num_to_select_final} points from combined results for Phase 2."); top_p_results_combined = combined_results[:num_to_select_final]
                log_with_elapsed_time(f"--- End {run_id_str} Phase 1bis ---")
            else:
                num_to_select = min(p_best_for_this_pass, len(valid_initial_results_p1)); log_with_elapsed_time(f"  Selecting top {num_to_select} points from Phase 1 for Phase 2."); top_p_results_combined = valid_initial_results_p1[:num_to_select]
            ...
            if st.session_state.debug_lbfgsb_run and pass_num == 1 and top_p_results_combined:
                ...
                log_with_elapsed_time(f"  Debug Starting Point EP (Cost ~{debug_cost_start:.3e}): {np.array2string(debug_start_ep, precision=4, max_line_width=120)}")
                ...
                try:
                    ...
                    log_with_elapsed_time("--- DEBUG L-BFGS-B Run Result ---"); log_message(f"Result Object:\n{debug_result}")
                     ...
                     log_with_elapsed_time(f"--- DEBUG L-BFGS-B Run FAILED ---"); log_message(f"Error during debug minimize call: {type(e_debug).__name__}: {e_debug}"); log_message(traceback.format_exc())
            ...
            if not selected_starts:
                ...
                if ep_ref_for_next_pass is not None: ep_this_pass = ep_ref_for_next_pass.copy()
                elif overall_best_ep_final is not None: ep_this_pass = overall_best_ep_final.copy()
                else: ep_this_pass = ep_nominal_glob.copy()
                ...
                num_layers_local = len(initial_best_ep_for_processing)
                lbfgsb_bounds = [(MIN_THICKNESS_PHYS_NM, None)] * num_layers_local
                args_for_cost_tuple_parallel = (inputs['nH_r'] + 1j*inputs['nH_i'], inputs['nL_r'] + 1j*inputs['nL_i'], inputs['nSub'] + 0j, l_vec_optim, active_targets, MIN_THICKNESS_PHYS_NM, False) # Debug False for parallel
            ...
            if ep_this_pass is not None and ep_this_pass.size > 0 and np.isfinite(cost_this_pass):
                if cost_this_pass < overall_best_cost_final:
                    ...
                    overall_best_cost_final = cost_this_pass; overall_best_ep_final = ep_this_pass.copy(); overall_best_layers = len(overall_best_ep_final)
                    final_successful_result_obj = result_obj_this_pass; new_best_found_this_pass = True
                ...
                log_with_elapsed_time(f"ERROR: {run_id_str} did not return a valid solution (cost: {cost_this_pass}).")
                st.session_state.current_status = f"{pass_status_prefix} - FAILED. {status_suffix}"; status_placeholder.warning(st.session_state.current_status)
                if overall_best_ep_final is not None: log_with_elapsed_time("Continuing optimization using previous best result."); ep_ref_for_next_pass = overall_best_ep_final.copy()
                else: raise RuntimeError(f"{run_id_str} failed and no prior successful result exists. Optimization aborted.")
        ...
        if overall_best_ep_final is None: raise RuntimeError("Optimization finished, but no valid result found before post-processing.")
        ...
        temp_args_for_cost_ar = (temp_nH_ar, temp_nL_ar, temp_nSub_c_ar, temp_l_vec_optim_ar, temp_active_targets_ar, MIN_THICKNESS_PHYS_NM, False) # Debug False
        current_ep_for_removal = overall_best_ep_final.copy()
        while auto_removed_count < max_auto_removals:
            if current_ep_for_removal is None or len(current_ep_for_removal) <= 1: log_with_elapsed_time("  Structure too short for further auto-removal."); break
            eligible_indices = np.where((current_ep_for_removal >= MIN_THICKNESS_PHYS_NM) & (current_ep_for_removal < auto_removal_threshold))[0]
            if eligible_indices.size > 0:
                min_val_in_eligible = np.min(current_ep_for_removal[eligible_indices]); thinnest_below_threshold_idx = np.where(current_ep_for_removal == min_val_in_eligible)[0][0]
                thinnest_below_threshold_val = min_val_in_eligible; log_with_elapsed_time(f"  Auto-removing layer {thinnest_below_threshold_idx + 1} (thickness {thinnest_below_threshold_val:.3f} nm < {auto_removal_threshold} nm)")
                status_suffix = f"| Best MSE: {overall_best_cost_final:.3e} | Layers: {overall_best_layers}"; st.session_state.current_status = f"Status: Auto-removing layer {auto_removed_count + 1}... {status_suffix}"
                status_placeholder.info(st.session_state.current_status)
                ...
                else: log_with_elapsed_time("    Auto-removal/re-optimization failed or structure unchanged. Stopping auto-removal."); break
            else: log_with_elapsed_time(f"  No layers found below {auto_removal_threshold} nm (and >= {MIN_THICKNESS_PHYS_NM} nm)."); break
        ...
        err_msg = f"ERROR (Input/Logic) during optimization: {e}"; log_with_elapsed_time(err_msg); st.error(err_msg)
        st.session_state.current_status = f"Status: Optimization Failed (Input/Logic Error)"; status_placeholder.error(st.session_state.current_status)
        optimization_successful = False; st.session_state.current_optimized_ep = None; st.session_state.optimization_ran_since_nominal_change = False
        st.session_state.optimized_qwot_display = "Error Input/Optim Logic"
    except Exception as e:
        err_msg = f"ERROR (Unexpected) during optimization: {type(e).__name__}: {e}"; tb_msg = traceback.format_exc()
        log_with_elapsed_time(err_msg); log_with_elapsed_time(tb_msg); print(err_msg); print(tb_msg); st.error(f"{err_msg}. See log/console for details.")
        st.session_state.current_status = f"Status: Optimization Failed (Unexpected Error)"; status_placeholder.error(st.session_state.current_status)
        optimization_successful = False; st.session_state.current_optimized_ep = None; st.session_state.optimization_ran_since_nominal_change = False
        st.session_state.optimized_qwot_display = "Optim Error Unexpected"
    finally:
        progress_placeholder.empty(); st.session_state.optim_start_time = None
...
if remove_button:
    ...
    if current_ep is None or not optim_ran or len(current_ep) <= 1:
        ...
        try:
            ...
            args_for_cost_tuple = (nH, nL, nSub_c, l_vec_optim, active_targets, MIN_THICKNESS_PHYS_NM, False) # Debug False
            ...
            if structure_actually_changed:
                log_message("  Layer removal successful. Updating structure and display."); st.session_state.current_optimized_ep = new_ep.copy(); removal_successful = True
                final_qwot_str = "QWOT Error"
                ...
                log_message("  No layer removed or structure unchanged."); st.info("Could not find suitable layer or removal/re-optimization failed/unchanged.")
            ...
            err_msg = f"ERROR (Input/Logic) during layer removal: {e}"; log_message(err_msg); st.error(err_msg); st.session_state.current_status = "Status: Removal Failed (Input/Logic Error)"; status_placeholder.error(st.session_state.current_status)
        except Exception as e:
            err_msg = f"ERROR (Unexpected) in layer removal: {type(e).__name__}: {e}"; tb_msg = traceback.format_exc(); log_message(err_msg); log_message(tb_msg); print(err_msg); print(tb_msg); st.error(f"{err_msg}. See log/console for details."); st.session_state.current_status = "Status: Removal Failed (Unexpected Error)"; status_placeholder.error(st.session_state.current_status)
        finally:
            progress_placeholder.empty(); update_display_info(st.session_state.current_optimized_ep if removal_successful else current_ep); st.rerun()
...
if set_nominal_button:
    ...
    if current_ep is None or not optim_ran or len(current_ep) == 0:
        log_message("ERROR: No optimized design available to set as nominal."); st.error("No valid optimized design is currently loaded."); st.session_state.current_status = "Status: Set Nominal Failed (No Design)"; status_placeholder.error(st.session_state.current_status)
    else:
        try:
            ...
            if np.any(np.isnan(optimized_qwots)): final_qwot_str = ""; log_message("Warning: QWOT calculation resulted in NaN. Cannot set nominal QWOT string."); st.warning("Could not calculate valid QWOT multipliers (NaN). Nominal QWOT field not updated.")
            else: final_qwot_str = ", ".join([f"{q:.6f}" for q in optimized_qwots]); st.session_state.emp_str_input = final_qwot_str; log_message(f"Nominal QWOT string updated in state: {final_qwot_str}"); st.success("Current optimized design set as new Nominal Structure (QWOT). Optimized state cleared.")
            ...
            err_msg = f"ERROR during 'Set Nominal' operation: {type(e).__name__}: {e}"; log_message(err_msg); st.error(f"An error occurred setting nominal: {e}"); st.session_state.current_status = "Status: Set Nominal Failed (Error)"; status_placeholder.error(st.session_state.current_status)
        finally: st.rerun()
